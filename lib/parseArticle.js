...
function replaceVisualReferences(text) {
  return text
    .replace(/\((?:See|see)\s+["'“”][^"'“”]*["'“”]\.?\)/g, "You can find this visual in the full article.")
    .replace(/\((?:See|see)[^)]+\)/g, "You can find this visual in the full article.")
    .replace(/\bsee the figure below\b/gi, "you can see this in the full article")
    .replace(/\bsee (?:figure|chart|table)\s+\d+\b/gi, "you can see this in the full article")
    .replace(/\bas shown in the (?:chart|figure|table)\b/gi, "as described in the full article")
    .replace(/\(Find out([^)]*)interactive quiz below\.\)/gi, "(Find out$1interactive quiz in the original article.)");
}

function cleanupText(text) {
  let next = text.replace(/\[\d+\]/g, "");
  next = replaceVisualReferences(next);
